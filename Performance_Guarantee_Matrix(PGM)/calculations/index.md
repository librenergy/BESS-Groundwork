...
One standalone sheet per guarantee mechanism, each carrying the complete calculation conditions so it can be reviewed independently. Built by the [`performance-guarantee-matrix`](/.agents/skills/performance-guarantee-matrix/SKILL.md) skill; name them `calc-{contract}-{metric}.md`.
