...
3. **Review each one, one document at a time.** Run the `project-document-review` skill per document to produce its companions beside the source file — a `summary.md` (key provisions and requirements, cited to clause), a **`definitions.md`** (every term *that document* defines, in its own words), and a `guarantees.md` wherever it carries guarantees, warranties, or LDs. Suggested order — design basis, offtake agreement, OEM service agreement, interconnection agreement, O&M agreement(s), EPC contract, then the SCADA sets and OEM equipment specs. This is the step that gives every session below its context: they stop being blank-page interviews and become reviews of what the contracts already say.
...
| [Project Documentation](./Project_Documentation/index.md) | [`project-document-review`](./.agents/skills/project-document-review/SKILL.md) | Domain folder structure for client-provided source documents (offtake, LTSA, EPC, interconnection, O&M, OEM, SCADA, design), each reviewed into a `summary.md` and — where guarantees exist — a `guarantees.md` beside the original file. |
