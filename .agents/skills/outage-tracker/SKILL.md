---
name: outage-tracker
title: Outage Tracker — Commercial Outage & Availability Event Accounting — Facilitated Session
type: Skill
description: Build or update the project's Outage Tracker, the commercial outage/derate event record (GADS-aligned taxonomy) that feeds every contractual availability calculation. Use whenever the user wants to track commercial outage events (ISO/offtaker-reported outages, excused events, planned maintenance, grid outages), define or tag outage categories and event types, reconcile availability records between the OEM CMMS, the offtaker/ISO view, and the data platform, set partial-outage/derate accounting rules, prepare or contest availability LD calculations, or get ready for NERC GADS reporting. Requires the project EIM, the Metrics Tree (availability formulas), and the Performance Guarantee Matrix (availability guarantees); run those skills first if missing.
---

# Outage Tracker: Commercial Outage & Availability Event Accounting (Facilitated Session)
...
Template: `Data_Product(DP)/Outage_Tracker/outage-tracker.md`. The tracker holds every **commercially significant** event (anything reported to the ISO/offtaker, anything needing an excused-event verdict, planned maintenance, grid outages) while sub-commercial equipment events stay in the CMMS. The premise behind the taxonomy: NERC GADS is the industry's event-accounting standard (Conventional, Wind, Solar; standalone BESS pending, storage today only as co-located groups in GADS Solar), so the tracker's event taxonomy is **GADS-aligned from day one**: one authoritative event record, tagged once, feeding every contractual availability calculation, exportable when NERC opens BESS reporting. The availability *formulas* live in `Metrics_Tree(MT)/metrics-tree.md`; this document owns the **event record** they depend on.

## Prerequisites

- Project EIM (identifies the ledgers and who writes them: GOP → offtaker/OMS notices; OEM → CMMS; telemetry → owner platform).
- the Metrics Tree for the availability formula variants; Performance Guarantee Matrix for which guarantee each variant serves. Capture ad hoc if missing, and flag.
...
1. `Data_Product(DP)/Outage_Tracker/outage-tracker.md` populated in place (version bumped); folder `todo.md`/`log.md` updated.
