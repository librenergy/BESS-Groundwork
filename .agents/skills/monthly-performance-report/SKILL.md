...
2. Walk the template's sections; keep/cut/add per project. For each kept section, verify its feeding source exists in the Data Interface Register and its metrics exist in the Metrics Tree (`Metrics_Tree(MT)/metrics-tree.md`). Missing feeds become the automation backlog.
