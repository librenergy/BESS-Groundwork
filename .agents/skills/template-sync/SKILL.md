...
| **2 — Status-bearing structure** (template shape + clone status interleaved) | `README.md`, `INSTRUCTIONS.md`, root `index.md`, `Project_Documentation/index.md`, `Data_Product(DP)/index.md`, `Definitions_Taxonomy(DT)/taxonomy.md` structural sections — plus **any template document the clone has not yet populated** (still carries `{{PLACEHOLDER}}`s), which may be taken wholesale | Diff, then merge by hand with the user — never overwrite |
