...
**Every domain folder carries an `instructions.md`** — the declared starting point for reviewing that kind of document, layered on the mechanical workflow in the skill. Five worked archetypes ship with the template, one per document class: [`PPA/`](PPA/instructions.md) (offtake — the owner *gives* the guarantees, read as the defendant), [`BSA/`](BSA/instructions.md) (supply — what was bought, at what rating, warranted against what), [`LTSA/`](LTSA/instructions.md) (service — the owner *holds* the guarantees, read as the claimant), [`OEM/`](OEM/instructions.md) (technical documentation, not a contract), and [`Design/`](Design/instructions.md) (drawings and design basis — a light-touch reference extraction, not an engineering check). Write one for any other domain on its first review, modelled on those.
